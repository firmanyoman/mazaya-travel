  ...
  return (
    <main className="min-h-screen">
      <header className="sticky top-0 z-10 border-b border-[var(--border)] bg-[color:color-mix(in_srgb,var(--background)_92%,white)] backdrop-blur">
        <div className="mx-auto flex max-w-[var(--container)] items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <div>
            ...
            <p className="text-sm text-[var(--muted)]">Foundation shell</p>
          </div>
          <a
            href="#hero"
            className="rounded-[var(--radius-control)] bg-[var(--primary)] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[var(--primary-hover)]"
          >
            Daftar Sekarang
          </a>
        </div>
      ...
      <section id="hero" className="px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-24">
        <div className="mx-auto grid max-w-[var(--container)] gap-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)] lg:items-center">
          <div className="space-y-6">
            <span className="inline-flex rounded-[var(--radius-pill)] bg-[var(--primary-soft)] px-4 py-2 text-sm font-semibold text-[var(--primary)]">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <a
                href="#next-sections"
                className="inline-flex items-center justify-center rounded-[var(--radius-control)] bg-[var(--primary)] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[var(--primary-hover)]"
              >
                Daftar Sekarang
              </a>
              <a
                href="#next-sections"
                className="inline-flex items-center justify-center rounded-[var(--radius-control)] border border-[var(--border)] bg-[var(--surface)] px-5 py-3 text-sm font-semibold text-[var(--primary)] transition hover:bg-[var(--primary-soft)]"
              >
                WhatsApp Konsultasi
              </a>
            </div>
            <ul className="flex flex-wrap gap-3">
              {trustBadges.map((badge) => (
                <li
                  key={badge}
                  className="rounded-[var(--radius-pill)] bg-[var(--surface)] px-4 py-2 text-sm font-medium text-[var(--foreground)] ring-1 ring-[var(--border)]"
                >
                  {badge}
                </li>
              ))}
      ...
      <section id="next-sections" className="px-4 pb-12 sm:px-6 sm:pb-16 lg:px-8 lg:pb-24">
        <div className="mx-auto max-w-[var(--container)] rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-6 sm:p-8 lg:p-10">
          <div className="mb-6 flex items-center justify-between gap-4">
            ...
            <span className="hidden rounded-[var(--radius-pill)] bg-[var(--brand-yellow)] px-4 py-2 text-sm font-semibold text-[var(--foreground)] sm:inline-flex">
              Design-token ready
            </span>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {sectionTitles.map((title, index) => (
              <article
                key={title}
                className="rounded-[calc(var(--radius-card)-8px)] border border-[var(--border)] bg-[var(--background)] p-5"
              >
                <p className="text-sm font-semibold text-[var(--primary)]">Section {index + 2}</p>
